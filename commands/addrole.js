...
exports.run = async (client, message, args, db) => {
    ...
    await db.collection('guilds').doc(message.guild.id).get().then(async (q) => {
        if(q.exists) {
            ...
            if(addToUser.roles.cache.has(role.id)) return message.reply(`${addToUser} already has the role ${role}!`);

            let embed = new MessageEmbed()
                .setAuthor('Member Edited', client.user.displayAvatarURL())
                .setColor('#b163e6')
                .setDescription(`${addToUser} has been added to the role ${role}`)

            addToUser.roles.add(role);
